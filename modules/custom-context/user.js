...
exports.user = function (session) {
    return {
                    ...
                    db.table('core', 'pd_users', session).Update(data, function (output) { 
                        if (output.meta.success == true) {
                            utils.sendMail(
                                args.name, 
                                data.c_email, 
                                `<p>Вас зарегистрировали на ресурсе <b>${args.name}!!!</b><br />
                                Для завершения процедуры регистрации перейдите по ссылке <a href="${args.site}">${args.site}</a> и введите учетные данные:
                                <ul><li>Логин: ${login}</li><li>Пароль: ${password}</li></ul></p>
                                <p>Чтобы открыть инструкцию перейдите по <a href="${args.site_help}">ссылке</a>.</p>`, 
                                function(err, info) {
                                    if(err) {
        ...
        passwordReset: function(data, callback) {
            authorizeDb.passwordReset(data.c_login, data.c_password, function(verify) {
